...
const CHAIN_ID =
  process.env.CHAIN_ID !== undefined ? parseInt(process.env.CHAIN_ID, 10) : CHAIN_IDS.POLYGON; // Polygon chain id
const NETWORK_ID =
  process.env.NETWORK_ID !== undefined ? parseInt(process.env.NETWORK_ID, 10) : CHAIN_ID;
...
async function checkLinkAllowance(contract) {
  try {
    ...
    if (parseFloat(allowance) > 0) {
      ...
      try {
        const signed = await currentlySelectedWeb3Client.eth.accounts.signTransaction(
          tx,
...
async function setCurrentWeb3Client(newWeb3ClientIndex) {
  ...
  const [aggregatorAddress, callbacksAddress, tradingAddress, vaultAddress, linkAddress] =
    await Promise.all([
  ...
  appLogger.info(
    'New web3 client selection completed. Took: ' + (performance.now() - executionStartTime) + 'ms'
  );
}
...
function createWeb3Client(providerIndex, providerUrl) {
  ...
  web3Client.eth.subscribe('newBlockHeaders').on('data', async (header) => {
    ...
    const blockDiff =
      currentlySelectedWeb3ClientIndex === -1
        ? newBlockNumber
        : newBlockNumber - currentWeb3ClientBlocks[currentlySelectedWeb3ClientIndex];
...
for (
  ...
  web3Clients.push(
    createWeb3Client(web3ProviderUrlIndex, WEB3_PROVIDER_URLS[web3ProviderUrlIndex])
  );
}
...
async function fetchTradingVariables() {
  ...
  try {
    ...
    appLogger.info(
      `Done fetching trading variables; took ${performance.now() - executionStart}ms.`
    );
  ...
  async function fetchBorrowingFees() {
    ...
    const borrowingFeesGroupIds = [
      ...new Set(
        borrowingFeesPairs.map((value) => value.groups.map((value) => value.groupIndex)).flat()
      ),
    ].sort((a, b) => a - b);
    ...
    const borrowingFeesGroupsInfo =
      borrowingFeesGroupIds.length > 0
        ? await borrowingFeesContract.methods
            .getGroups(
              Array.from(Array(+borrowingFeesGroupIds[borrowingFeesGroupIds.length - 1] + 1).keys())
            )
            .call()
        : [];
...
async function fetchOpenTrades() {
  ...
  try {
    ...
    const newTradesLastUpdated = new Map(
      await populateTradesLastUpdated(openLimitOrders, pairTrades, USE_MULTICALL)
    );
    ...
    appLogger.info(
      `Fetched ${knownOpenTrades.size} total open trade(s) in ${performance.now() - start}ms.`
    );
  ...
  async function populateTradesLastUpdated(openLimitOrders, pairTrades, useMulticall) {
    ...
    if (useMulticall) {
      ...
      for (let i = 0; i < openLimitOrders.length; i += batchSize) {
        ...
        const batchLastUpdated = await Promise.all(
          batch.map((order) =>
            fetchTradeLastUpdated(order.trader, order.pairIndex, order.index, TRADE_TYPE.LIMIT)
          )
        );
      ...
      for (let i = 0; i < pairTrades.length; i += batchSize) {
        ...
        const batchLastUpdated = await Promise.all(
          batch.map((order) =>
            fetchTradeLastUpdated(order.trader, order.pairIndex, order.index, TRADE_TYPE.MARKET)
          )
        );
    ...
    appLogger.info(
      `Fetched last updated info for ${tLastUpdated.length + olLastUpdated.length} trade(s).`
    );
  ...
  try {
    ...
    eventSubBorrowingFeesContext.borrowing = borrowingFeesContract.events
      ...
      .on('data', (event) => {
        ...
        if (
          eventName !== 'PairAccFeesUpdated' &&
          eventName !== 'GroupAccFeesUpdated' &&
          eventName !== 'GroupOiUpdated' &&
          eventName !== 'PairMaxOiUpdated'
        ) {
          return;
        }
        // If no confirmation delay, then execute immediately without timer
...
function watchPricingStream() {
  ...
  socket.onmessage = (msg) => {
    ...
    handleOnMessageAsync()
      .catch((error) => {
        appLogger.error('Unhandled error occurred when handling pricing stream message!', {
          error,
        });
      })
      .finally(() => {
              ...
              const orderTransaction = createTransaction(
                {
                  ...
                  data: tradingContract.methods
                    .executeNftOrder(packNft(orderType, trader, pairIndex, index, 0, 0))
                    .encodeABI(),
                },
                true
              );
  ...
  if (network.gasMode === GAS_MODE.EIP1559) {
    ...
    return {
      gasPrice: Web3.utils.toHex(
        gasPriceBn.mul(Web3.utils.toBN(priorityMultiplier)).div(Web3.utils.toBN(100))
      ),
    };
  }
