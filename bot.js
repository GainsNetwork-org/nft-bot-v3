...
const MAX_FEE_PER_GAS_WEI_HEX =
		(process.env.MAX_GAS_PRICE_GWEI ?? "").length > 0
			? Web3.utils.toHex(parseInt(process.env.MAX_GAS_PRICE_GWEI, 10) * 1e9)
			: 0,
	MAX_GAS_PER_TRANSACTION_HEX = Web3.utils.toHex(
		parseInt(process.env.MAX_GAS_PER_TRANSACTION, 10)
	),
	EVENT_CONFIRMATIONS_MS =
		parseFloat(process.env.EVENT_CONFIRMATIONS_SEC) * 1000,
	...
	OPEN_TRADES_REFRESH_MS =
		(process.env.OPEN_TRADES_REFRESH_SEC ?? "").length > 0
			? parseFloat(process.env.OPEN_TRADES_REFRESH_SEC) * 1000
			: 120,
	GAS_REFRESH_INTERVAL_MS =
		(process.env.GAS_REFRESH_INTERVAL_SEC ?? "").length > 0
			? parseFloat(process.env.GAS_REFRESH_INTERVAL_SEC) * 1000
			: 3,
	...
	ENABLE_CONCURRENT_PRICE_UPDATE_PROCESSING = process.env
		.ENABLE_CONCURRENT_PRICE_UPDATE_PROCESSING
		? process.env.ENABLE_CONCURRENT_PRICE_UPDATE_PROCESSING === "true"
		: true,
	USE_MULTICALL = process.env.USE_MULTICALL
		? process.env.USE_MULTICALL === "true"
		: true,
	l1BlockFetchIntervalMs = process.env.L1_BLOCK_FETCH_INTERVAL_MS
		? +process.env.L1_BLOCK_FETCH_INTERVAL_MS
		: undefined,
	MAX_RETRIES =
		process.env.MAX_RETRIES && !isNaN(+process.env.MAX_RETRIES)
...
const CHAIN_ID =
	process.env.CHAIN_ID !== undefined
		...
		: CHAIN_IDS.POLYGON; // Polygon chain id
const NETWORK_ID =
	process.env.NETWORK_ID !== undefined
...
async function checkLinkAllowance(contract) {
	try {
		const allowance = await linkContract.methods
			.allowance(process.env.PUBLIC_KEY, contract)
			.call();
		const key =
			contract === process.env.STORAGE_ADDRESS ? "storage" : "rewards";
		...
		if (parseFloat(allowance) > 0) {
			...
			try {
				const signed =
					await currentlySelectedWeb3Client.eth.accounts.signTransaction(
						tx,
						process.env.PRIVATE_KEY
					);

				await currentlySelectedWeb3Client.eth.sendSignedTransaction(
...
async function setCurrentWeb3Client(newWeb3ClientIndex) {
	appLogger.info(
		"Switching web3 client to " +
			WEB3_PROVIDER_URLS[newWeb3ClientIndex] +
			" (#" +
			newWeb3ClientIndex +
			")..."
	);
	...
	storageContract = new newWeb3Client.eth.Contract(
		abis.STORAGE,
	...
	callbacksContract = new newWeb3Client.eth.Contract(
		abis.CALLBACKS,
		callbacksAddress
	);
	tradingContract = new newWeb3Client.eth.Contract(
		abis.TRADING,
		tradingAddress
	);
	vaultContract = new newWeb3Client.eth.Contract(abis.VAULT, vaultAddress);
	pairInfosContract = new newWeb3Client.eth.Contract(
		abis.PAIR_INFOS,
		process.env.PAIR_INFOS_ADDRESS
	);

	const aggregatorContract = new newWeb3Client.eth.Contract(
		abis.AGGREGATOR,
		aggregatorAddress
	);

	const [pairsStorageAddress, nftRewardsAddress, borrowingFeesAddress] =
		await Promise.all([
	...
	borrowingFeesContract = new newWeb3Client.eth.Contract(
		abis.BORROWING_FEES,
		borrowingFeesAddress
	);
	pairsStorageContract = new newWeb3Client.eth.Contract(
		abis.PAIRS_STORAGE,
		pairsStorageAddress
	);
	nftRewardsContract = new newWeb3Client.eth.Contract(
		abis.NFT_REWARDS,
		nftRewardsAddress
	);
	...
	appLogger.info(
		"New web3 client selection completed. Took: " +
			(performance.now() - executionStartTime) +
			"ms"
	);
}

async function checkContractApprovals(
	addresses = [process.env.STORAGE_ADDRESS]
) {
...
function createWeb3Client(providerIndex, providerUrl) {
	...
	web3Client.eth.subscribe("newBlockHeaders").on("data", async (header) => {
		...
		if (newBlockNumber === null) {
			appLogger.debug(
				`Received unfinished block from provider ${providerUrl}; ignoring...`
			);
		...
		if (
			l1BlockFetchIntervalMs !== undefined &&
			Date.now() - prevL1BlockFetchTimeMs > l1BlockFetchIntervalMs
		) {
			prevL1BlockFetchTimeMs = Date.now();
			try {
				const block = await web3Client.eth.getBlock(newBlockNumber);
				const _currentL1BlockNumber = Web3.utils.hexToNumber(
					block.l1BlockNumber
				);

				if (_currentL1BlockNumber > latestL1Block)
					latestL1Block = _currentL1BlockNumber;

				if (currentL1Blocks[providerIndex] !== _currentL1BlockNumber) {
					currentL1Blocks[providerIndex] = _currentL1BlockNumber;
					appLogger.debug(
						`New L1 block received ${_currentL1BlockNumber} from provider ${providerUrl}...`
					);
				}
			} catch (error) {
				appLogger.error(
					`An error occurred attempting to fetch L1 block number for block ${newBlockNumber}`,
					{
		...
		appLogger.debug(
			`New block received ${newBlockNumber} from provider ${providerUrl}...`
		);
		...
		const blockDiff =
			currentlySelectedWeb3ClientIndex === -1
				? newBlockNumber
				: newBlockNumber -
				  currentWeb3ClientBlocks[currentlySelectedWeb3ClientIndex];
...
for (
	var web3ProviderUrlIndex = 0;
	web3ProviderUrlIndex < WEB3_PROVIDER_URLS.length;
	web3ProviderUrlIndex++
) {
	web3Clients.push(
		createWeb3Client(
			web3ProviderUrlIndex,
			WEB3_PROVIDER_URLS[web3ProviderUrlIndex]
		)
	);
}

let MAX_PROVIDER_BLOCK_DRIFT =
	(process.env.MAX_PROVIDER_BLOCK_DRIFT ?? "").length > 0
		? parseInt(process.env.MAX_PROVIDER_BLOCK_DRIFT, 10)
		: 2;

if (MAX_PROVIDER_BLOCK_DRIFT < 1) {
	appLogger.warn(
		`MAX_PROVIDER_BLOCK_DRIFT is set to ${MAX_PROVIDER_BLOCK_DRIFT}; setting to minimum of 1.`
	);
...
setInterval(() => {
	if (currentlySelectedWeb3ClientIndex === -1) {
		...
		appLogger.info(
			`Current Web3 Client: ${currentlySelectedWeb3Client.currentProvider.url} (#${currentlySelectedWeb3ClientIndex})`
		);
	}
	...
	executionStats = {
		...executionStats,
		uptime: DateTime.now()
			.diff(DateTime.fromJSDate(executionStats.startTime), [
				"days",
				"hours",
				"minutes",
				"seconds",
		...
		try {
			...
			setTimeout(async () => {
				doNextLatestGasPricesFetch().catch((error) => {
					appLogger.error(
						`An error occurred attempting to fetch latest gas prices; will be tried again in ${GAS_REFRESH_INTERVAL_MS}.`,
						{
							error,
						}
					);
			...
			try {
				...
				if (NETWORK.gasMode === GAS_MODE.EIP1559) {
					...
					priorityTransactionMaxPriorityFeePerGas = Math.round(
						Math.max(
							Math.round(gasPriceData.fast.maxPriorityFee) *
				...
				appLogger.error(
					"Error while fetching gas prices from gas station!",
					error
				);
			}
		} else {
...
const FETCH_TRADING_VARIABLES_REFRESH_INTERVAL_MS =
	(process.env.FETCH_TRADING_VARIABLES_REFRESH_INTERVAL_SEC ?? "").length > 0
		? parseFloat(process.env.FETCH_TRADING_VARIABLES_REFRESH_INTERVAL_SEC) *
		  1000
		: 60 * 1000;
...
async function fetchTradingVariables() {
	...
	if (currentTradingVariablesFetchPromise !== null) {
		appLogger.warn(
			`A current fetchTradingVariables call was already in progress, just awaiting that...`
		);
	...
	try {
		...
		appLogger.info(
			`Done fetching trading variables; took ${
	...
	async function fetchPairInfos(pairsCount) {
		const [pairInfos, newMaxNegativePnlOnOpenP, maxLeverage] =
			await Promise.all([
				pairInfosContract.methods
					.getPairInfos([...Array(parseInt(pairsCount)).keys()])
					.call(),
	...
	async function fetchBorrowingFees() {
		...
		const [borrowingFeesGroups, groupExponents] = [
			borrowingFeesGroupsInfo["0"],
			borrowingFeesGroupsInfo["1"],
		];
	...
	async function fetchOiWindows(pairLength) {
		const { startTs, windowsDuration, windowsCount } =
			await borrowingFeesContract.methods.getOiWindowsSettings().call();
		...
		const windowsToCheck = [...Array(5).keys()]
			.map((i) => currWindowId - i)
			.filter((v) => v > -1);
		...
		const oiWindowsTemp = (
			await Promise.all(
				[...Array(parseInt(pairLength)).keys()].map((_, pairIndex) =>
					borrowingFeesContract.methods
						.getOiWindows(
							oiWindowsSettings.windowsDuration,
							pairIndex,
							windowsToCheck
						)
						.call()
			...
			pairWindows.reduce(
				(acc, curr, i) => ({ ...acc, [windowsToCheck[i]]: curr }),
				{}
			)
		);
...
async function fetchOpenTrades() {
	...
	try {
		...
		const [openLimitOrders, pairTrades] = await Promise.all([
			...
			openLimitOrders
				...
				.map((trade) => [
					buildTradeIdentifier(
						...
						trade.pairIndex,
						trade.index,
						trade.openPrice === undefined
					),
					trade,
				])
		);

		const newTradesLastUpdated = new Map(
			await populateTradesLastUpdated(
				openLimitOrders,
				pairTrades,
				USE_MULTICALL
			)
		...
		appLogger.info(
			`Fetched ${knownOpenTrades.size} total open trade(s) in ${
		...
		if (OPEN_TRADES_REFRESH_MS !== 0) {
			appLogger.debug(
				`Scheduling auto-refresh of open trades in for ${OPEN_TRADES_REFRESH_MS}ms from now.`
			);
	...
	function scheduleRetryFetchOpenTrades() {
		if (fetchOpenTradesRetryTimerId !== null) {
			appLogger.debug(
				"Already scheduled retry fetching open trades; will retry shortly!"
			);
			...
			openLimitOrders.map(async (olo) => {
				const [type, tradeData] = await Promise.all([
					nftRewardsContract.methods
						.openLimitOrderTypes(olo.trader, olo.pairIndex, olo.index)
						.call(),
					callbacksContract.methods
						.tradeData(olo.trader, olo.pairIndex, olo.index, 1)
						.call(),
				]);
				...
				return {
					...
					type,
					maxSlippageP:
						parseFloat(tradeData.maxSlippageP.toString()) / 1e10 || 1,
				};
			})
		);

		appLogger.info(
			`Fetched ${openLimitOrdersWithTypes.length} open limit order(s).`
		);
	...
	async function fetchOpenPairTrades() {
		...
		appLogger.info(
			`Fetched ${allOpenPairTrades.length} new open pair trade(s).`
		);

		return allOpenPairTrades;
	}

	async function populateTradesLastUpdated(
		openLimitOrders,
		pairTrades,
		useMulticall
	) {
		...
		if (useMulticall) {
			...
			const callbacksContractMulticall = new Contract(
				callbacksContract.options.address,
				abis.CALLBACKS
			);
			...
			for (let i = 0; i < openLimitOrders.length; i += batchSize) {
				...
				olLastUpdated = olLastUpdated
					? olLastUpdated.concat(batchLastUpdated)
					: batchLastUpdated;
			}
			for (let i = 0; i < pairTrades.length; i += batchSize) {
				...
				tLastUpdated = tLastUpdated
					? tLastUpdated.concat(batchLastUpdated)
					: batchLastUpdated;
			}
		}

		appLogger.info(
			`Fetched last updated info for ${
				tLastUpdated.length + olLastUpdated.length
			} trade(s).`
		);
		return transformLastUpdated(
			openLimitOrders,
			olLastUpdated,
			pairTrades,
			tLastUpdated
...
async function fetchTradeLastUpdated(trader, pairIndex, index, tradeType) {
	const lastUpdated = await callbacksContract.methods
...
function watchLiveTradingEvents() {
	try {
		...
		if (eventSubPairInfos === null) {
			eventSubPairInfos = pairInfosContract.events
				.allEvents({ fromBlock: "latest" })
				.on("data", (event) => {
					const eventName = event.event;
		...
		if (
			eventSubBorrowingFeesContext.borrowing !== null &&
			eventSubBorrowingFeesContext.borrowing.id !== null
		) {
			eventSubBorrowingFeesContext.borrowing.unsubscribe();
		}

		eventSubBorrowingFeesContext.borrowing = borrowingFeesContract.events
			.allEvents({ fromBlock: "latest" })
			.on("data", (event) => {
				const eventName = event.event;
...
async function synchronizeOpenTrades(event) {
	try {
		...
		appLogger.verbose(
			`Synchronizing open trades based on event ${eventName} from block ${event.blockNumber}...`
		);
		...
		if (eventName === "OpenLimitCanceled") {
			...
			if (existingKnownOpenTrade !== undefined) {
				...
				appLogger.verbose(
					`Synchronize open trades from event ${eventName}: Removed limit for ${tradeKey}`
				);
			} else {
				appLogger.verbose(
					`Synchronize open trades from event ${eventName}: Limit not found for ${tradeKey}`
				);
			}
		} else if (
			eventName === "OpenLimitPlaced" ||
			...
			if (hasOpenLimitOrder === false) {
				appLogger.warn(
					`Open limit order not found for ${trader}/${pairIndex}/${index}; ignoring!`
				);
			} else {
				const [limitOrder, type, lastUpdated, tradeData] = await Promise.all([
					...
					nftRewardsContract.methods
						.openLimitOrderTypes(trader, pairIndex, index)
						.call(),
					fetchTradeLastUpdated(trader, pairIndex, index, TRADE_TYPE.LIMIT),
					callbacksContract.methods
						.tradeData(trader, pairIndex, index, 1)
						.call(),
				]);
				...
				if (currentKnownOpenTrades.has(tradeKey)) {
					appLogger.verbose(
						`Synchronize open trades from event ${eventName}: Updating open limit order for ${tradeKey}`
					);
				} else {
					appLogger.verbose(
						`Synchronize open trades from event ${eventName}: Storing new open limit order for ${tradeKey}`
					);
				}
			...
			const [trade, tradeInfo, tradeInitialAccFees, lastUpdated] =
				await Promise.all([
					...
					storageContract.methods
						...
						.call(),
					borrowingFeesContract.methods
						.getTradeInitialAccFees(trader, pairIndex, index)
						.call(),
			...
			appLogger.verbose(
				`Synchronize open trades from event ${eventName}: Updated trade ${tradeKey}`
			);
			...
			if (eventName === "LimitExecuted") {
				const openTradeKey = buildTradeIdentifier(
					trader,
					pairIndex,
					limitIndex,
					true
				...
				if (currentKnownOpenTrades.has(openTradeKey)) {
					appLogger.verbose(
						`Synchronize open trades from event ${eventName}: Removed open limit trade ${openTradeKey}.`
					);
				...
				resetRetryCounter(
					buildTriggerIdentifier(trader, pairIndex, limitIndex, 3)
				);
			}
			...
			appLogger.info(
				`Synchronize open trades from event ${eventName}: Stored active trade ${tradeKey}`
			);
			...
			appLogger.info(
				`Synchronize open trades from event ${eventName}: event received for ${triggeredOrderTrackingInfoIdentifier}...`
			);
			...
			if (existingKnownOpenTrade !== undefined) {
				...
				tradesLastUpdated.delete(tradeKey);
				appLogger.debug(
					`Synchronize open trades from event ${eventName}: Removed ${tradeKey} from known open trades.`
				);
			} else {
				appLogger.debug(
			...
			const triggeredOrderDetails = triggeredOrders.get(
			...
			if (triggeredOrderDetails !== undefined) {
				...
				if (triggeredOrderDetails.transactionSent === true) {
					if (eventReturnValues.nftHolder === process.env.PUBLIC_KEY) {
						appLogger.info(
							`💰💰💰 SUCCESSFULLY TRIGGERED ORDER ${triggeredOrderTrackingInfoIdentifier} FIRST!!!`
						);
						...
						appLogger.info(
							`💰 SUCCESSFULLY TRIGGERED ORDER ${triggeredOrderTrackingInfoIdentifier} AS SAME BLOCK!!!`
						);
			...
			pairMaxLeverage.set(
				eventReturnValues.pairIndex,
				parseFloat(eventReturnValues.maxLeverage)
			);
			appLogger.verbose(
				`${eventName}: Set pairMaxLeverage for pair ${eventReturnValues.pairIndex} to ${eventReturnValues.maxLeverage}.`
			);
		}
...
async function refreshPairFundingFees(event) {
	...
	const newPairFundingFees = await pairInfosContract.methods
...
async function handleBorrowingFeesEvent(event) {
	try {
		if (event.event === "PairAccFeesUpdated") {
			...
			const { pairIndex, windowId, long, openInterest } =
				event.returnValues.oiWindowUpdate;
...
function watchPricingStream() {
	...
	socket.onclose = () => {
		appLogger.error(
			"Pricing stream websocket closed! Will attempt to reconnect in two seconds..."
		);
	...
	socket.onmessage = (msg) => {
		...
		if (currentKnownOpenTrades === null) {
			appLogger.debug(
				"Known open trades not yet loaded; unable to begin any processing yet!"
			);
		...
		if (spreadsP.length === 0) {
			appLogger.debug(
				"Spreads are not yet loaded; unable to process any trades!"
			);
		...
		if (!allowedLink.storage || !allowedLink.rewards) {
			appLogger.warn(
				"LINK is not currently allowed for the configured account; unable to process any trades!"
			);
		...
		if (
			ENABLE_CONCURRENT_PRICE_UPDATE_PROCESSING === false &&
		...
		handleOnMessageAsync()
			.catch((error) => {
				appLogger.error(
					"Unhandled error occurred when handling pricing stream message!",
					{ error }
				);
			})
			.finally(() => {
			...
			const statuses = await Promise.allSettled(
				[...currentKnownOpenTrades.values()].map(async (openTrade) => {
					...
					if (isPendingOpenLimitOrder === false) {
						...
						const posDai =
							parseFloat(openTrade.leverage) *
							parseFloat(openTrade.positionSize);
						...
						const newCollateralDai =
							collateralDai + parseFloat(openTrade.positionSize);

						const maxInterestDai = parseFloat(
							openInterests[openTrade.pairIndex].max
						);
						const maxCollateralDai = parseFloat(
							collaterals[openTrade.pairIndex].max
						);
						...
						if (
							...
							spreadWithPriceImpactP * openTrade.leverage <=
							...
							if (
								...
								(tradeType === "1" &&
									(buy ? price <= maxPrice : price >= minPrice)) ||
								(tradeType === "2" &&
									(buy ? price >= minPrice : price <= maxPrice))
							) {
								orderType = 3;
							} else {
					...
					if (!lastUpdated && orderType !== 2) {
						appLogger.warn(
							`Last updated information for ${openTradeKey} is not available`
						);
						return;
					}
					...
					if (triggeredOrders.has(triggeredOrderTrackingInfoIdentifier)) {
						appLogger.debug(
							`Order ${triggeredOrderTrackingInfoIdentifier} has already been triggered by us and is pending!`
						);
					...
					try {
						...
						if (!currentKnownOpenTrades.has(openTradeKey)) {
							appLogger.warn(
								`Trade ${openTradeKey} no longer exists in our known open trades list; skipping order!`
							);
						...
						appLogger.info(
							`🤞 Trying to trigger ${triggeredOrderTrackingInfoIdentifier}...`
						);
						...
						try {
							const orderTransaction = createTransaction(
								{
									...
									data: tradingContract.methods
										.executeNftOrder(
											packNft(orderType, trader, pairIndex, index, 0, 0)
										)
										.encodeABI(),
								},
								true
							...
							appLogger.info(
								`⚡️ Triggered order for ${triggeredOrderTrackingInfoIdentifier}.`
							);
						} catch (error) {
							const executionStatsTriggerErrors =
								executionStats.triggerErrors ?? {};
							const errorReason = error.reason ?? "UNKNOWN_TRANSACTION_ERROR";

							executionStatsTriggerErrors[errorReason] =
								(executionStatsTriggerErrors[errorReason] ?? 0) + 1;
							...
							switch (errorReason) {
								...
								default:
									...
									if (
										...
										appLogger.info(
											"Nonce refreshed and tracking of triggered order cleared so it can possibly be retried."
										);
									} else {
										appLogger.error(
	...
	async function claimRewards() {
		...
		const tx = createTransaction({
			...
			data: nftRewardsContract.methods
				.claimRewards(process.env.ORACLE_ADDRESS)
				.encodeABI(),
		});
...
function getTransactionGasFees(network, isPriority = false) {
	if (network.gasMode === GAS_MODE.EIP1559) {
		return {
			...
			maxFeePerGas: isPriority
				? MAX_FEE_PER_GAS_WEI_HEX
				: Web3.utils.toHex(standardTransactionGasFees.maxFee * 1e9),
		};
	} else if (network.gasMode === GAS_MODE.LEGACY) {
		...
		return {
			gasPrice: Web3.utils.toHex(
				gasPriceBn
					.mul(Web3.utils.toBN(priorityMultiplier))
