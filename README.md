...
| ENABLE_CONCURRENT_PRICE_UPDATE_PROCESSING | Whether or not the bot should allow multiple price updates to be processed concurrently. This allows for faster reaction time when triggering orders rather than waiting on previous price update processing to complete. This is experimental and defaults to `false` today. |
